...
const handleEditClick = (row) => {
  ...
  useEffect(() => {
    ...
    let colRef = query(collection(db, "prices"),where("email" ,"==", user.email));
    ...
    <>
      <div className="table-responsive">
        ...
        <div>
          ...
          <input id="name" value={selectedRow.name} disabled />
        </div>
        <div>
          <label htmlFor="price">Price:</label>
          <input id="price" value={selectedRow.price} />
        </div>
        <div>
          <label htmlFor="quantity">Quantity:</label>
<input id="quantity" value={selectedRow.quantity} />
        </div>
              ...
              return (
                <tr>
                  ...
                  <td>
                    {(data?.find((d) => {
                      if (d.name === ele.name) return d;
                    })?.quantity - ele?.quantity).toFixed(2)}
                  </td>
                </tr>
              );
            })}
